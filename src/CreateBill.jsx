...
const CreateBill = ({ onBack, onGenerateInvoice }) => {
  ...
  useEffect(() => {
    fetchSweets().then(setSweetsList).catch(() => setSweetsList([]));
  }, []);
                ...
                <tbody>
                  {items.map((item, idx) => (
                    <tr key={idx}>
                      <td>
                        <select
                          ...
                          onChange={e => {
                            ...
                            const sweetObj = sweetsList.find(s => s.name === sweetName);
                            handleItemChange(idx, 'sweet', sweetName);
                            if (sweetObj) {
                              handleItemChange(idx, 'rate', sweetObj.rate);
                            }
                          }}
                        >
                      ...
                      <td>
                        <input className="form-control" type="number" min="0" value={item.rate} onChange={e => handleItemChange(idx, 'rate', Number(e.target.value))} placeholder="Rate" readOnly={!!item.sweet} />
                      </td>
                      <td>
